...
class RubricService {
  ...
  static async getAllRubrics() : Promise<Response> {
    return fetch(BASE_API_URL + `/rubrics`, AuthUtils.requestHeaders);
  }

  static async getAssessmentRubrics(assessment: string) : Promise<Response> {
    return fetch(BASE_API_URL + `/rubrics/${assessment}`, AuthUtils.requestHeaders);
  }
  ...
  static async deleteRubric(rubricId: number) : Promise<Response> {
    ...
    return fetch(BASE_API_URL + `rubrics/${rubricId}`, requestOptions);
  }
